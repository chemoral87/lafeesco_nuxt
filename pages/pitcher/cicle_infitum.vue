...
export default {
  ...
  methods: {
    ...
    autoCorrelate(buf, sampleRate) {
      ...
      this.dBDisplay = (dB + 85).toFixed(1);
    ...
    drawNoteLines() {
      ...
      const detectedNoteName = this.noteDisplay.replace(/[0-9]/g, ""); // "C", "G#", etc.
      ...
      for (let midi = MIN_MIDI; midi <= MAX_MIDI; midi++) {
        ...
        const noteNameOnly = noteName.replace(/[0-9]/g, "");
        const isSameDetectedNote = noteNameOnly === detectedNoteName;

        ctx.font =
          isNearby && isInScale ? "18px sans-serif" : "16px sans-serif";
        ctx.lineWidth = isNearby
          ? 3
          : isInScale || isSameDetectedNote
          ? 2.5
          : 1.5;
        ...
        ctx.fillStyle = isSameDetectedNote ? "red" : "white";
    ...
    drawHistogram() {
      ...
      // Solo obtenemos estos datos una vez para el punto más reciente
      ...
      const noteName = this.getNoteName(Math.round(midi));
      const noteIndex = Math.round(midi) % 12;
      const baseFreq = freq;

      // Preparamos el texto que se mostrará en todas las octavas
      const displayText = `${noteName} (${freq.toFixed(0)} Hz)`;
      ...
      for (let octaveOffset = -2; octaveOffset <= 4; octaveOffset++) {
        ...
        // Dibujamos el punto (rojo para la octava principal, color de la nota para las demás)
        ctx.fillStyle = octaveOffset === 0 ? "red" : color;
        ctx.beginPath();
        ctx.arc(x, y, 2, 0, 2 * Math.PI);
        ctx.fill();

        // Dibujamos la misma etiqueta en todas las octavas visibles
        if (octaveOffset >= -2 && octaveOffset <= 4) {
          ctx.fillStyle = "red";
          ctx.fillText(displayText, x - textWidth - 10, y - 5);
        }
      }

      // Dibujamos el resto del histograma (puntos históricos)
      for (let i = 1; i < len; i++) {
