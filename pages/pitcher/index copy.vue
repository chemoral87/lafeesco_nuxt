  ...
  <v-container class="pa-4" style="max-width: 1000px">
    ...
    >
      <canvas
        ...
        width="900"
        height="400"
        style="display: block; margin: auto; background-color: white"
      ></canvas>
    </div>
  </v-container>
</template>
...
const A4_FREQ = 440;
const A4_MIDI = 69;
const TOTAL_NOTES = 15; // 12 notas normales + 3 semitonos extendidos
...
export default {
  ...
  methods: {
    ...
    drawNoteLines() {
      ...
      // Dibujamos 15 líneas (12 notas + 3 extendidas)
      for (let i = 0; i < TOTAL_NOTES; i++) {
        const noteIndex = i % 12;
        const y = height - i * (height / TOTAL_NOTES);
        const noteName = NOTE_SHORT_STRINGS[noteIndex];

        // Verificar si hay frecuencias cercanas a esta nota
        const isNearby = recentFreqs.some((f) => {
          if (!f) return false;
          const midi = this.freqToMidi(f);
          const currentNotePos = Math.round(midi) % 12;
          // Comparamos con la posición en el ciclo extendido
          return (
            currentNotePos === noteIndex &&
            Math.floor(midi / 12) === Math.floor(i / 12)
          );
        });
        ...
        ctx.fillStyle = isNearby ? COLORS[noteIndex] : "black";
        ...
        // Mostrar el nombre de la nota solo para la primera octava
        ctx.fillText(noteName, width - 40, y + 3);
        // if (i < 12) {
        //   ctx.fillText(noteName, width - 40, y + 3);
        // } else {
        //   // Para notas extendidas, mostrar un indicador de octava
        //   ctx.fillText(noteName + "↑", width - 40, y + 3);
        // }
      }
    },
    drawHistogram() {
      ...
      // Ajustamos el rango de octavas para cubrir las notas extendidas
      for (let octaveOffset = -1; octaveOffset <= 2; octaveOffset++) {
        ...
        // Mapeamos a la posición en el rango extendido (0-14)
        const y =
          height -
          (((shiftedMidi % 12) + (octaveOffset + 1) * 12) % TOTAL_NOTES) *
            (height / TOTAL_NOTES);
