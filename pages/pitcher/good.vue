...
export default {
  ...
  methods: {
    ...
    autoCorrelate(buf, sampleRate) {
      ...
      // Calcular dB SPL (Sound Pressure Level) según estándar NIOSH
      // Referencia: 0 dB SPL = 20 micropascales (umbral de audición humana)
      // Asumimos que el máximo digital (1.0) corresponde a ~120 dB SPL
      const dB_SPL = 20 * Math.log10(rms / 0.00002); // 0.00002 = 20 micropascales

      // Aplicar ponderación de frecuencia (aquí necesitarías implementar filtros A/C/Z)
      // Para simplificar, usaremos el valor sin ponderar
      this.dBDisplay = Math.max(0, dB_SPL).toFixed(1);

      // Mostrar advertencia si supera el threshold NIOSH (80 dB)
      if (dB_SPL > 80) {
        console.warn(
      ...
      // Si el sonido es muy bajo (por debajo de 30 dB), no procesar frecuencia
      if (dB_SPL < 30) {
        this.freqDisplay = "--";
        this.noteDisplay = "--";
        return -1;
    ...
    drawHistogram() {
      ...
      // Datos de la frecuencia actual detectada
      ...
      for (let octaveOffset = -2; octaveOffset <= 4; octaveOffset++) {
        ...
        if (isSameNoteFamily) {
          // Notas de la misma familia (todas las octavas) en azul
          pointColor = textColor = "white"; // Azul
        } else {
          // Usar color del array COLORS basado en el índice de la nota
      ...
      // Dibujamos el historial (puntos anteriores)
      for (let i = 1; i < len; i++) {
        const { freq, midi } = this.history[i];
        if (!freq || freq < 20 || freq > 2000) continue;
        this.drawHistoryPoints(i, freq, midi, spacing, scaleNoteIndices);
    ...
    drawNoteLines() {
      ...
      // Obtener nota base actual si existe
      let currentNoteBase = "";
      ...
      for (let midi = MIN_MIDI; midi <= MAX_MIDI; midi++) {
        ...
        if (isCurrentNoteFamily) {
          if (isInScale) {
            // Nota actual y en escala - rosa
            ctx.strokeStyle = "red"; // Rosa más claro cuando no está cerca
            ctx.fillStyle = "red";
          } else {
            // Nota actual pero no en escala - amarillo
            ctx.strokeStyle = "#FFEE99"; // Amarillo más claro cuando no está cerca
            ctx.fillStyle = "#FFEE99";
          }
        } else if (isNearby) {
          // Notas cercanas: usar color del array COLORS
    ...
    drawHistoryPoints(i, freq, midi, spacing, scaleNoteIndices) {
      ...
      for (let octaveOffset = -2; octaveOffset <= 4; octaveOffset++) {
        ...
        // Usar color del array COLORS atenuado para el historial
        const color = COLORS[shiftedNoteIndex];
        const fadedColor = this.lightenColor(color, 50); // Atenuar color
