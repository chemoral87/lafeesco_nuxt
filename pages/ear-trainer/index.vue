<template>
  <v-container fluid>
    <v-row dense>
      <v-col cols="12">
        <EarTrainerNotes @remove="removeNoteList($event)" :notes="playNotesList"></EarTrainerNotes>
      </v-col>
      <v-col cols="2" sm="1">
    ...
    <v-row dense>
      ...
      <v-col cols="12">
        <EarTrainerGameList @deleteItem="deleteItem($event)" @verify="verify($event)" :games="games"></EarTrainerGameList>
      </v-col>
    </v-row>
  </v-container>
</template>
...
export default {
  ...
  methods: {
    ...
    play() {
      ...
      let max_tries = 2
      let n = 0
      while (n < max_tries && this.games[0] && JSON.stringify(this.games[0].notes) == JSON.stringify(notes)) {
    ...
    getRandomNotes() {
      ...
      while (n < this.max_tones) {
        let note = NoteList.splice(Math.floor(Math.random() * NoteList.length), 1)
        n++
        notes.push(note[0])
      }
