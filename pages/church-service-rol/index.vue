  ...
  <v-container fluid class="pa-2">
    ...
    <v-row dense ref="captureElement" v-if="range_view == 'service'">
      <v-col
        ...
        lg="3"
        class="px-1"
        v-for="service in church_services_filtered"
        :key="service.id"
      >
        <v-card
        ...
        <v-card class="pa-1">
          <v-card-title class="pa-1 text-body-2">
            <v-avatar size="29" class="mr-1">
              <img
                :src="person.photo"
                :alt="`${person.name} ${person.paternal_surname}`"
              />
            </v-avatar>
...
export default {
  ...
  computed: {
    ...
    attendant_ministry_events() {
      let attendant_list = [];
      this.church_services_filtered.forEach(
        ({ ministries, event_date, event_name, event_name_color }) => {
          ministries.forEach(({ attendants, ...ministry }) => {
            attendants.forEach((attendant) => {
              let attendant_item = attendant_list.find(
              ...
              let ministry_item = attendant_item.ministries.find(
      ...
      this.church_services.forEach((service) => {
        ...
        if (this.selectedMinistries.length > 0) {
          ...
          filtered_ministries.forEach((ministry) => {
            if (this.selectedMinistries.length != 1)
