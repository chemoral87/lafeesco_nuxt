    ...
    <v-card>
      ...
      <v-form ref="form" @submit.prevent="save">
        <v-card-text>
          <v-row dense>
            <v-col cols="12">
              <v-select label="Contactado" @change="item.call_type_id = null" v-model="item.was_contacted" :items="was_contacted_items" item-text="text" item-value="value" outlined hide-details />
            </v-col>
            <v-col cols="12">
              <v-select label="Tipo de Llamada" ref="call_type" v-model="item.call_type_id" :items="call_types_filtered" item-text="name" item-value="id" :rules="[rules.required]" outlined :hide-details=" $refs.call_type && $refs.call_type.valid" />
            </v-col>
            <v-col cols="12">
